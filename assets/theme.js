...
class SlideCus extends SlideSection {
  ...
  initSlide() {
    // Tránh init khi element chưa hiển thị (offsetWidth = 0)
    if (this.offsetWidth === 0) {
      setTimeout(() => this.initSlide(), 100);
      return;
    }

    // Gọi hàm gốc từ SlideSection để khởi tạo Swiper
    super.initSlide();

    // Nếu Swiper đã được khởi tạo, override lại breakpoints theo ý muốn
    if (this.globalSlide) {
      const spacing = this.globalSlide.params.spaceBetween || 20;
  ...
  handleResize() {
    ...
    this._resizeTimeout = setTimeout(() => {
      if (this.globalSlide) {
        this.globalSlide.destroy(true, true);
        this.globalSlide = null;
      }
      this.initSlide();
    }, 100);
